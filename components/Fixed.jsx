...
const Fixed = () => {
  ...
  return (
    <div className="hidden sticky top-2 sm:flex justify-center align-center flex-wrap h-4/5 w-4/5 my-20 z-auto">
      {IMAGES && IMAGES.map((image, index) => (
        console.log(image),
        <div key={index} className="w-1/4 h-2/3 relative">
          <Image alt="image" src={image} width={400} height={300} loading="lazy" />
        </div>
      ))}
    </div>
  )
